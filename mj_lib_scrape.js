    ...
    request(url, function(error, response, html){
        if(!error){
            ...
            $('table tr').each(function(){
                ...
                link = $(this).find('a').attr('href'); json.link = "http://lib.mju.ac.kr/" + link
                ...
                if((json.no.length != 0) && (json.no != "공지")) array_json.push(json);
            });
            array_json.reverse();
        }
        ...
        fs.writeFile('mj_lib_output.json', JSON.stringify(array_json, null, 6), function(err){
        });

        //파일 읽고 내용 로그
        fs.readFile('./mj_lib_output.json', 'utf8', function (err, data) {
            Observer.count({category: "mj_lib"}, function(err,count){
                if(count == 0){
                    var first = new Observer({category: "mj_lib"});
                    first.save();                       
                }else{
                    Observer.findOne({category: "mj_lib"}, function(err, doc){
